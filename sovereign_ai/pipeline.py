...
class SovereignPipeline:
    ...
    def __init__(self, config: Config):
        ...
        os.environ["SOVEREIGN_ENV"] = config.environment
        ...
        try:
            ...
            raise RuntimeError("NLI evaluator dependencies are required for verification") from exc
    ...
    async def verify(self, query: str, context: str, answer: str) -> dict[str, Any]:
        ...
        if not eval_res.get("passed", False) and self.config.fail_closed:
            eval_res["safe_answer"] = (
                "[Sovereign Access Denied] Answer failed grounding verification."
            )
        else:
