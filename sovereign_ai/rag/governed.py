...
class GovernedRetriever:
    ...
    def __init__(
        ...
        self.policy_engine = PolicyEngine(
            policy_path, trusted_public_key=trusted_policy_key, strict_mode=strict_policy
        )
        ...
        self.audit_logger = SovereignAuditLogger(
            base_dir=base_dir, tenant_id=tenant_id, anchor=anchor, attest=attest
        )
    ...
    def search(
        self, query: str, intent: str = "general", top_k: int = 100, rerank_top_k: int = 5
    ) -> Tuple[List[SearchResult], PolicyDecision]:
...
class AsyncGovernedRetriever:
    ...
    async def search(
        ...
        await asyncio.to_thread(
            self.audit_logger.log, "async_rag_retrieval", self.principal, audit_data
        )
        return results_to_return, decision
