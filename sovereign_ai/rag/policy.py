...
class PolicyEngine:
    ...
    def __init__(
        ...
        trusted_public_key: Optional[Union[bytes, str]] = None,
        ...
        try:
            ...
            if len(self.trusted_public_key) == 32:  # Ed25519
                ...
                pub = ec.EllipticCurvePublicKey.from_encoded_point(
                    ec.SECP256R1(), self.trusted_public_key
                )
    ...
    def _load_policy(self) -> Dict[str, Any]:
        ...
        if not self.policy_path or not self.policy_path.exists():
            logger.warning(
                f"Policy file NOT FOUND at {self.policy_path}. Using 'Deny-All' safety default."
            )
        ...
        if self.trusted_public_key or self.strict_mode:
            if not self.verify_signature():
                logger.critical(
                    "FORCED POLICY DENIAL: Signature invalid or missing in strict mode."
                )
                return {
                    "allow": [],
                    "deny": [{"classification": "all"}],
    ...
    def evaluate_request(
        self, request: AccessRequest, results: List[SearchResult]
    ) -> PolicyDecision:
        ...
        for res in results:
            ...
            if is_authorized:
                ...
                if res.score >= min_score:
                    ...
                    logger.debug(
                        f"Chunk {res.chunk_id} filtered by score: {res.score} < {min_score}"
                    )
                    denied_ids.append(res.chunk_id)
            else:
    ...
    def _match_rule(
        self, rule: Dict[str, Any], principal: Principal, chunk_meta: Dict[str, Any], intent: str
    ) -> bool:
    ...
    def _generate_reason(self, action: str, allowed_count: int, denied_count: int) -> str:
        ...
        return (
            f"Sovereign {action.capitalize()}: {allowed_count} authorized, {denied_count} filtered."
        )
