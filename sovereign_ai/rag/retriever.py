...
class FTS5Retriever:
    ...
    def ingest(self, docs: List[Document], chunk_size: int = 1000, chunk_overlap: int = 200):
        ...
        try:
            for doc in docs:
                ...
                if doc.department:
                    meta["department"] = doc.department
                if doc.tenant_id:
    ...
    def search(
        self, query: str, top_k: int = 5, snippet_tokens: int = DEFAULT_SNIPPET_TOKENS
    ) -> List[SearchResult]:
...
class AsyncFTS5Retriever:
    ...
    async def ingest(self, docs: List[Document], chunk_size: int = 1000, chunk_overlap: int = 200):
        """Asynchronously ingest documents by offloading to a separate retriever instance."""
        import asyncio

        def _blocking_ingest():
            # Initialize inside the worker thread to ensure the connection is thread-local
            sync_retriever = FTS5Retriever(
                db_path=str(self.store.db_path), password=self.store.password
            )
            try:
                sync_retriever.ingest(docs, chunk_size, chunk_overlap)
            finally:
                sync_retriever.store.close()
