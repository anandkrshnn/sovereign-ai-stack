...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        asyncio.create_task(asyncio.to_thread(SovereignAIAgent))
...
async def get_agent_async():
    ...
    async with transition_lock:
        if current_agent is None:
            ...
            current_agent = await asyncio.to_thread(SovereignAIAgent)
...
def get_agent():
    ...
    global current_agent
    if current_agent is None:
        current_agent = SovereignAIAgent()
    return current_agent
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        request=request,
        name="index.html",
        context={"stats": stats, "current_vault": current_vault_name},
    )
...
async def create_vault(request: Request):
    ...
    try:
        ...
        current_agent = await asyncio.to_thread(
            SovereignAIAgent, vault_root=vault_path, password=password
        )
    ...
    async def event_generator():
        try:
            ...
            full_response = await asyncio.wait_for(
                asyncio.to_thread(current_agent.chat, user_input), timeout=180.0
            )
...
async def confirm_permission(request: Request):
    ...
    if not request_id or request_id not in pending_requests:
        ...
        return {
            "success": False,
            "message": "Invalid or expired request ID. Please try sending your instruction again.",
        }
    ...
    try:
        result = agent.broker.confirm_permission(
            intent=pending["intent"], resource=pending["resource"], approved=approved
        )
    except Exception as e:
...
async def get_memory_items(status: Optional[str] = None):
    ...
    try:
        items = await asyncio.to_thread(
            agent.memory_service.lancedb_store.get_memory_items, status=status
        )
    ...
    try:
        if action == "forget":
            await asyncio.to_thread(
                agent.memory_service.lancedb_store.delete_memory_item, memory_id
            )
        elif action == "approve":
            await asyncio.to_thread(
                agent.memory_service.lancedb_store.update_memory_status, memory_id, "approved"
            )
        elif action == "reject":
            await asyncio.to_thread(
                agent.memory_service.lancedb_store.update_memory_status, memory_id, "rejected"
            )
...
async def optimize_memory():
    ...
    try:
        ...
        episodes = await asyncio.to_thread(
            agent.memory_service.lancedb_store.get_recent_episodes, limit=50
        )
        for ep in episodes:
            await asyncio.to_thread(
...
async def get_trust_status():
    ...
    return {"active_rules": learned, "candidate_rules": pending}
...
async def get_policies():
    ...
    active = agent.broker.policy_engine.get_active_rules()
    candidates = agent.broker.policy_engine.get_candidate_rules()
    return {"active": list(active.values()), "candidates": list(candidates.values())}
...
async def simulate_rule(request: Request):
    ...
    result = await asyncio.to_thread(
        agent.broker.policy_engine.simulate_policy_impact, intent, pattern, effect
    )
    return result
...
async def get_trace(trace_id: str):
    ...
    try:
        ...
        if not trace_entry:
            raise HTTPException(
                status_code=404, detail=f"Trace {trace_id} not found in episodic storage."
            )
...
async def download_report():
    ...
    try:
        ...
        return JSONResponse(
            content=data,
            headers={
                "Content-Disposition": f"attachment; filename=compliance_report_{current_vault_name}.json"
            },
        )
    except Exception as e:
        return {"error": str(e)}
...
async def verify_audit_integrity():
    ...
    is_valid = await asyncio.to_thread(
        AuditChainManager.verify_log_integrity, log_path, agent.vault.key_manager
    )

    return {"success": True, "is_valid": is_valid, "log_path": str(log_path)}
...
async def bridge_confirm(request: Request):
    ...
    try:
        ...
        return {"success": True, "response": str(final_result)}
    except Exception as e:
        return {"success": False, "message": str(e)}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Start the SovereignAIAgent Dashboard")
    ...
    if args.daemon_mode:
        print(f"Starting SovereignAIAgent Daemon on port {args.port}...")

    uvicorn.run("sovereign_ai.agent.api.app:app", host=args.host, port=args.port, reload=False)
