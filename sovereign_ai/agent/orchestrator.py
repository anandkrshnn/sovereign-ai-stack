...
class SovereignAgent:
    ...
    async def execute_tool(
        ...
        v_result = await loop.run_in_executor(
            ...
            partial(
                ...
                context=context,
                answer=f"Agent wants to call {tool_name} with parameters: {canonical_args}",
            ),
        )
        ...
        signed_v_event = await self._audit_sign(
            "verification_gate", state, verify_record.model_dump(), wait=True
        )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"SHIELD_ORPHAN: Forensic write for {state.action_id} is running in background due to task cancellation."
                )
                raise
            raise
    ...
    async def _audit_sign(
        self, action: str, state: AgentState, data: Dict[str, Any], wait: bool = False
    ) -> Optional[Any]:
