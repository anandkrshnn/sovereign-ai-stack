    ...
    def _provision_key() -> str:
        ...
        try:
            keyring.set_password(
                SecureKeyManager.SERVICE_NAME, SecureKeyManager.KEY_NAME, key_bytes.decode()
            )
            print("[SecureKeyManager] Key successfully provisioned and stored in Windows Enclave.")
        except Exception as e:
            print(
                f"[SecureKeyManager] WARNING: Failed to store key in keyring ({e}). Key will be volatile."
            )
