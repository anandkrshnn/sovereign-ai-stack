...
class LocalPermissionBroker:
    ...
    def __init__(
            ...
            try:
                ...
                if master_passphrase:
                    try:
                        ...
                        print(
                            "[Broker] v8.0 RECOVERY SUCCESS: Audit key restored from vault-staple."
                        )
                    except:
                        print(
                            f"[Broker] WARNING: Secure Key Manager failed ({e}). Proceeding unencrypted."
                        )
                else:
                    print(
                        f"[Broker] WARNING: Secure Key Manager failed ({e}). Proceeding unencrypted."
        ...
        self.policy_engine = PolicyEngine(
            self, db_path=str(policies_path), key_manager=self.key_manager
        )

        # Phase 2: Initialize Hash Chain
        mode = "encrypted" if self.key_manager and self.key_manager.is_encrypted() else "plaintext"
        self.last_audit_hash = AuditChainManager.get_last_hash(
            self.audit_log_path, key_manager=self.key_manager, mode=mode
        )
        ...
        self.scanner_manager = ScannerManager(
            config=self.config, allowlist=self.policy_engine.get_allowlist()
        )
    ...
    def request_permission(
        self, intent: str, resource: str, context: str = "", trace_id: str = None
    ) -> dict:
        """
        ...
        try:
            ...
            if violations:
                ...
                for v in violations:
                    ...
                    redacted_context = redacted_context.replace(
                        v._raw_match or "", v.redact_and_clear()
                    )
        ...
        # Suspicious markers
        is_suspicious = (
            ".." in resource or os.path.isabs(resource) or (os.name == "nt" and ":" in resource[:3])
        )
        ...
        is_in_sandbox = (
            normalized_resource.startswith("sandbox/") or normalized_resource == "sandbox"
        )
        ...
        try:
            self._append_audit(
                request_id, intent, resource, False, None, msg, context, trace_id=trace_id
            )
        except Exception:
            return {
                "granted": False,
    ...
    def confirm_permission(
        self, intent: str, resource: str, approved: bool, context: str = "", trace_id: str = None
    ) -> dict:
        """
        ...
        if rule and not rule.get("requires_confirmation", True):
            ...
            result["message"] = (
                f"Pattern learned! Future requests for {resource} will auto-approve."
            )
        else:
    ...
    def promote_to_always_allow(
        self, intent: str, resource_pattern: str, description: str = None
    ) -> dict:
        ...
        return {
            "success": True,
            "rule_id": rule_id,
            "message": f"Successfully trusted {resource_pattern} forever.",
        }

    def promote_to_never_allow(
        self, intent: str, resource_pattern: str, description: str = None
    ) -> dict:
        ...
        return {
            "success": True,
            "rule_id": rule_id,
            "message": f"Successfully BLOCKED {resource_pattern} forever.",
        }
            ...
            with open(self.audit_log_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        # Filter criteria
                        if (
                            entry.get("intent") == intent
                            and entry.get("resource", "").startswith(resource_pattern)
                            and entry.get("granted") is True
    ...
    def _append_audit(
        ...
        with self._vault_lock:
            ...
            try:
                ...
                if (
                    os.path.exists(self.audit_log_path)
                    and os.path.getsize(self.audit_log_path) > 1024 * 1024
                ):
    ...
    def _rotate_audit_log(self):
        ...
        print(
            f"[Broker] Audit log rotation {self._archive_seq} triggered. Archiving to {archive_name}"
        )
        ...
        if (self.audit_log_path.with_suffix(".anchor")).exists():
            shutil.move(
                self.audit_log_path.with_suffix(".anchor"), archive_path.with_suffix(".anchor")
            )
