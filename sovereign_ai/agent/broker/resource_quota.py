    ...
    def check_and_update(self, intent: str, resource_size: int = 0) -> bool:
        ...
        if intent in ["write_file", "append_to_file"]:
            ...
            if new_ops > self.MAX_WRITE_OPS:
                print(
                    f"[ResourceQuota] REJECTED: Write operations limit ({self.MAX_WRITE_OPS}) exceeded."
                )
                return False

            if new_bytes > self.MAX_WRITE_BYTES:
                print(
                    f"[ResourceQuota] REJECTED: Write byte limit ({self.MAX_WRITE_BYTES // 1024 // 1024}MB) exceeded."
                )
                return False
