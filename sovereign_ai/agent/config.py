...
class Config:
    ...
    bridge_secret: str = os.getenv(
        "SOVEREIGN_AI_BRIDGE_SECRET", "localagent-default-secret-change-me"
    )
    bridge_enabled: bool = os.getenv("SOVEREIGN_AI_BRIDGE_ENABLED", "True").lower() == "true"

    # Phase 2: Security Hardening
    secret_scanner_entropy_threshold: float = float(
        os.getenv("SECRET_SCANNER_ENTROPY_THRESHOLD", "4.5")
    )
    secret_scanner_fail_open: bool = (
        os.getenv("SECRET_SCANNER_FAIL_OPEN", "False").lower() == "true"
    )
