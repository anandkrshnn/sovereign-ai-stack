...
def _handle_ipc_message(msg: Any) -> Dict[str, Any]:
    ...
    if command == "ping":
        ...
        return {"status": "online", "agent_loaded": current_agent is not None, "pid": os.getpid()}
    elif command == "chat":
        ...
        result = current_agent.broker.confirm_permission(
            pending["intent"], pending["resource"], approved
        )
        ...
        if approved and result.get("granted"):
            try:
                observation = current_agent._execute_tool(
                    pending["tool_name"], pending["arguments"], token=result.get("token")
                )
                return {
                    "status": "success",
                    "response": f"[Airlock Resolved] Observation: {observation}",
                }
            except Exception as e:
                return {"error": str(e)}
