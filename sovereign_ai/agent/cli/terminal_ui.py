...
def render_airlock_modal(violation: Dict[str, Any]):
    ...
    table.add_row("[bold cyan]SOURCE[/bold cyan]", ": [italic]Local / CLI Prompt[/italic]")

    risk_color = "red" if violation.get("risk", "Low") == "High" else "yellow"
    table.add_row(
        "[bold cyan]RISK[/bold cyan]",
        f": [{risk_color}]{violation.get('risk', 'Moderate')}[/{risk_color}]",
    )
