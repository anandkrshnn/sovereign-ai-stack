...
def start(
    daemon: bool = typer.Option(
        False, "--daemon", "-d", help="Run SovereignAIAgent in the background as a daemon"
    ),
    api_token: str = typer.Option(
        None, "--api-token", help="Specify a static API token for the daemon"
    ),
    port: int = typer.Option(8000, "--port", "-p", help="Port to bind the API to"),
):
    """Start the SovereignAIAgent core and dashboard."""
    ...
    if daemon:
        ...
        console.print(
            Panel("[bold green]Starting SovereignAIAgent in foreground mode...[/bold green]")
        )
...
def policy_cmd(
    ...
    signature: Optional[str] = typer.Option(
        None, "--signature", help="Cryptographic signature file (.sig)"
    ),
    key_file: Optional[str] = typer.Option(
        None, "--key-file", help="Path to administration key for verification"
    ),
):
...
def forensics_cmd(
    ...
    key_file: Optional[str] = typer.Option(
        None, "--key-file", help="Path to vault.key for headless decryption"
    ),
):
...
def interact(
    ...
    no_timeout: bool = typer.Option(
        False, "--no-timeout", help="Disable reject timeout for testing"
    ),
):
...
def request_cmd(
    ...
    profile: str = typer.Option(
        "plaintext", "--profile", help="Security profile (plaintext|encrypted)"
    ),
):
