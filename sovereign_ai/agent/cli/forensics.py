...
def forensics_export(
    ...
    """
    console.print("[STATUS] [bold cyan]Forensics Export Service[/bold cyan]")
    ...
    if key_file:
        ...
        if key_path.exists():
            ...
            console.print(
                f"[yellow]Warning: Key file {key_file} not found. Defaulting to prompt.[/yellow]"
            )
    ...
    try:
        ...
        console.print(
            "[red]Invalid time format for --since (use 24h, 7d, etc.). Defaulting to 24h.[/red]"
        )
        ...
        with open(log_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    if ts_str:
                        entry_time = datetime.fromisoformat(ts_str.replace("Z", "+00:00")).replace(
                            tzinfo=None
                        )
    ...
    console.print(
        f"Statistics: Total Entries: {len(exported_entries)} (Skipped {skipped_count} older entries)"
    )
