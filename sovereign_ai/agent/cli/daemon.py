...
def start_daemon_process(token: Optional[str] = None, port: int = 8000) -> None:
    ...
    if pid_file.exists():
        try:
            ...
            if psutil.pid_exists(existing_pid):
                console.print(
                    f"[bold red][FAIL] Daemon already running (PID {existing_pid})[/bold red]"
                )
                return
        except ValueError:
    ...
    if os.name == "nt":
        ...
        proc = subprocess.Popen(
            cmd, stdout=log_handle, stderr=subprocess.STDOUT, start_new_session=True, close_fds=True
        )
...
def stop_daemon() -> None:
    ...
    try:
        ...
        if psutil.pid_exists(pid):
            ...
            console.print("[bold green][STOP] Daemon stopped.[/bold green]")
        else:
