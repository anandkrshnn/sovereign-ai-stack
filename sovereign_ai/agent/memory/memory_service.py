...
class MemoryService:
    def __init__(
        self, broker: LocalPermissionBroker, lancedb_path: str, duckdb_path: str, key_manager=None
    ):
    ...
    def get_governed_context(self, user_query: str, session_context: Dict[str, Any]):
        ...
        perm = self.broker.request_permission(
            intent="read_memory", resource="semantic_memory", context=session_context
        )
    ...
    def refresh_hot_cache(self):
        ...
        if not hasattr(self, "hot_memory"):
            return
