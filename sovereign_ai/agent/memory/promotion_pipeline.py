    ...
    def run_cycle(self, episode_id: str, content: str, trace_id: str = "NO_TRACE"):
        ...
        with self._lock:
            if self.active_queue >= PROMOTION_PIPELINE_MAX_QUEUED:
                print(
                    f"[Promotion] Queue flooded (> {PROMOTION_PIPELINE_MAX_QUEUED}). Evicting extraction to protect LPB."
                )
                return
            self.active_queue += 1
        ...
        try:
            candidates = self.extract_candidates(
                {"episode_id": episode_id, "content_text": content, "trace_id": trace_id}
            )
            ...
            for candidate in candidates:
                ...
                if not is_duplicate:
                    ...
                    vector = (
                        embedder.encode(candidate["body"]).tolist() if embedder else [0.0] * 384
                    )

                    # Persist to LanceDB
                    self.lancedb_store.promote_memory(candidate, vector)
                    print(
                        f"[Promotion] New candidate saved: {candidate['body'][:40]}... ({candidate['sensitivity']})"
                    )
                else:
