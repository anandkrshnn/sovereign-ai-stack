...
def ask(query, principal, tenant, verify):
    ...
    config = Config(principal=principal, tenant_id=tenant, enable_verification=verify)
...
def ingest(path, tenant):
    ...
    doc = Document(doc_id=str(uuid.uuid4()), source=path, content=content, tenant_id=tenant)
...
@click.option(
    "--backend", default="auto", type=click.Choice(["auto", "mock", "tpm2_linux", "tpm2_windows"])
)
