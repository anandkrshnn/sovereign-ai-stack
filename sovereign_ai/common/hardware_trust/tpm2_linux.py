...
def secure_zero(data: bytearray | bytes):
    ...
    if isinstance(data, (bytes, bytearray)):
        ctypes.memset(
            ctypes.byref((ctypes.c_char * len(data)).from_buffer_copy(data)), 0, len(data)
        )
...
class TPM2LinuxAnchor(SecureAnchor):
    ...
    def __init__(self, tenant_id: str, aik_handle: int = 0x81000002):
        if not HAS_PYTSS:
            raise ImportError(
                "python-tpm2-pytss not installed. Run: pip install sovereign-ai-stack[tpm2]"
            )
        ...
        try:
            ...
            logger.warning(
                "🚨 [SIMULATION] TPM 2.0 Hardware initialization failed or AIK Handle 0x%x not found. Falling back to Software Simulation mode.",
                self.aik_handle,
            )
    ...
    def sign_payload(self, payload: bytes) -> bytes:
        ...
        try:
            ...
            if "0x00000081" in str(e) or "handle" in str(e).lower():
                logger.warning(
                    f"AIK Handle 0x{self.aik_handle:x} not found. Using simulation fallback."
                )
    ...
    def _read_pcr(self, pcr_index: int) -> str:
        ...
        try:
            ...
            pcr_sel = TPML_PCR_SELECTION(
                pcrSelections=[TPMS_PCR_SELECTION(hash=TPM2_ALG.SHA256, pcrSelect=[pcr_index])]
            )
