...
class SecureKeyManager:
    ...
    def get_or_create_session_key(self) -> bytes:
        ...
        stored_key = keyring.get_password(
            self.SERVICE_NAME, f"{self.keyring_username}_{self.SESSION_KEY_NAME}"
        )
    ...
    def get_or_create_signing_key(self) -> ed25519.Ed25519PrivateKey:
        ...
        stored_key_pem = keyring.get_password(
            self.SERVICE_NAME, f"{self.keyring_username}_{self.SIGNING_KEY_NAME}"
        )

        if stored_key_pem:
            # Deserialize from PEM
            private_key = serialization.load_pem_private_key(stored_key_pem.encode(), password=None)
            return private_key
        ...
        # Store in keyring
        keyring.set_password(
            self.SERVICE_NAME, f"{self.keyring_username}_{self.SIGNING_KEY_NAME}", private_key_pem
        )
