...
class NLIAdaptiveGate(SovereignAirlock):
    ...
    def set_thresholds(self, entailment: float, contradiction: float) -> None:
        ...
        logger.info(
            "NLI thresholds dynamically updated. Entailment: %s, Contradiction: %s",
            entailment,
            contradiction,
        )
    ...
    async def verify(self, claim: str, context: List[str]) -> AirlockResult:
        ...
        reason = (
            "Entailment Verified"
            if is_safe
            else f"Insufficient Entailment (Score: {entailment:.3f} < {self.entailment_threshold})"
        )
    ...
    def verify_consistency(
        ...
        if not current_knowledge:
            ...
            return (
                "ACCEPT",
                {"contradiction": 0.0, "entailment": 1.0, "neutral": 0.0},
                "Initial context insertion (Inherent trust)",
            )
        ...
        if contradiction >= self.contradiction_threshold:
            return (
                "REJECT",
                probs,
                f"Active Contradiction Detected (Contradiction: {contradiction:.3f} >= {self.contradiction_threshold})",
            )
        ...
        if entailment >= self.entailment_threshold:
            return (
                "ACCEPT",
                probs,
                f"Strong Logical Entailment (Entailment: {entailment:.3f} >= {self.entailment_threshold})",
            )
        ...
        return (
            "QUARANTINE",
            probs,
            f"Unverified / Neutral Information (Contradiction: {contradiction:.3f}, Entailment: {entailment:.3f})",
        )
