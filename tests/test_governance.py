...
from sovereign_ai.common.audit import Principal as AuditPrincipal
from sovereign_ai.common.audit import SovereignAuditLogger
from sovereign_ai.rag.policy import AccessRequest, PolicyEngine, Principal
from sovereign_ai.rag.schemas import PolicyDecision, SearchResult
...
def policy_file(tmp_path):
    ...
    policy_data = {
        ...
        "allow": [{"classifications": ["public"], "roles": ["analyst"]}],
        "deny": [{"departments": ["hr"]}],
        "limits": {"max_results": 2, "min_score": 0.5},
    }
...
def test_policy_engine_enforcement(policy_file, mock_results):
    ...
    principal = Principal(
        id="test-analyst", tenant_id="tenant-a", roles=["analyst"], classifications=["public"]
    )
...
def test_governed_retriever_airlock(tmp_path, policy_file):
    ...
    with patch("sovereign_ai.rag.governed.FTS5Retriever") as mock_fts_cls:
        ...
        retriever = GovernedRetriever(
            str(db_path), policy_file, principal="analyst", tenant_id="tenant-a", roles=["analyst"]
        )
