...
def test_evidence_bundle_pydantic_validation():
    ...
    with pytest.raises(ValueError):
        EvidenceBundle(
            nonce="short", merkle_root="0x1", bundle_signature="sig"  # Should fail min_length=32
        )
...
def test_attestation_roundtrip_hardware_native():
    ...
    # 4. Verifier
    ref_values = {"app_hash": "app_hash_v1", "aik_public_key_pem": public_pem}
    verifier = AttestationVerifier(ref_values)

    bundle = EvidenceBundle(
        nonce=challenge_nonce, merkle_root="0x789", quote=quote, bundle_signature="outer_sig"
    )
