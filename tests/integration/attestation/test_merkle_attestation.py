...
async def main():
    ...
    config = Config(tenant_id="test_tenant", enable_attestation=True)
        ...
        for line in f:
            ...
            if event.get("action") == "MERKLE_CHECKPOINT":
                ...
                if quote:
                    ...
                    print(
                        f"Nonce matches root: {quote.get('nonce') == event['event_data']['merkle_root']}"
                    )
                else:
